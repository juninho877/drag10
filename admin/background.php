...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if ($posted_bg_type && isset($background_types[$posted_bg_type])) {
        ...
        if ($_SESSION["role"] !== 'admin') {
            $result = $user->incrementImageChangeCount($userId, 'background');
            if (!$result['success']) {
                ...
                header("Location: background.php?tipo=" . $current_bg_key);
                exit();
            }
        }
...
if ($_SESSION["role"] !== 'admin') {
    ...
    if ($userData) {
        $imageLimits = [
            'background_changes_today' => $userData['background_changes_today'],
            'background_change_limit' => $userData['background_change_limit']
        ];
    }
}
...
<div class="card mt-6">
    ...
    <div class="card-body">
        <div class="flex items-center gap-3">
            <div class="limit-indicator <?php echo $imageLimits['background_changes_today'] >= $imageLimits['background_change_limit'] ? 'limit-reached' : ''; ?>">
                <div class="limit-count"><?php echo $imageLimits['background_changes_today']; ?>/<?php echo $imageLimits['background_change_limit']; ?></div>
                <div class="limit-label">trocas hoje</div>
            </div>
            <div class="limit-info">
                <p class="text-sm">
                    <?php if ($imageLimits['background_changes_today'] >= $imageLimits['background_change_limit']): ?>
                        <span class="text-danger-500">Você atingiu o limite diário de trocas de fundo.</span>
                    <?php else: ?>
                        <span class="text-success-500">Você ainda pode trocar o fundo <?php echo $imageLimits['background_change_limit'] - $imageLimits['background_changes_today']; ?> vezes hoje.</span>
                    <?php endif; ?>
                </p>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="form-group">
                ...
                <label for="bg-selector" class="form-label">Tamanho Recomendado: 1440x2030 </label>
