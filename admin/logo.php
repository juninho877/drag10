...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if ($posted_logo_type && isset($logo_types[$posted_logo_type])) {
        ...
        if ($_SESSION["role"] !== 'admin') {
            $result = $user->incrementImageChangeCount($userId, 'logo');
            if (!$result['success']) {
                ...
                header("Location: logo.php?tipo=" . $current_logo_key);
                exit();
            }
        }
...
if ($_SESSION["role"] !== 'admin') {
    ...
    if ($userData) {
        $imageLimits = [
            'logo_changes_today' => $userData['logo_changes_today'],
            'logo_change_limit' => $userData['logo_change_limit']
        ];
    }
}
...
<div class="card mt-6">
    ...
    <div class="card-body">
        <div class="flex items-center gap-3">
            <div class="limit-indicator <?php echo $imageLimits['logo_changes_today'] >= $imageLimits['logo_change_limit'] ? 'limit-reached' : ''; ?>">
                <div class="limit-count"><?php echo $imageLimits['logo_changes_today']; ?>/<?php echo $imageLimits['logo_change_limit']; ?></div>
                <div class="limit-label">trocas hoje</div>
            </div>
            <div class="limit-info">
                <p class="text-sm">
                    <?php if ($imageLimits['logo_changes_today'] >= $imageLimits['logo_change_limit']): ?>
                        <span class="text-danger-500">Você atingiu o limite diário de trocas de logo.</span>
                    <?php else: ?>
                        <span class="text-success-500">Você ainda pode trocar o logo <?php echo $imageLimits['logo_change_limit'] - $imageLimits['logo_changes_today']; ?> vezes hoje.</span>
                    <?php endif; ?>
                </p>
