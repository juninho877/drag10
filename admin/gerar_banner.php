...
try {
    ...
    $larguraMaximaTitulo = $imageWidth - 480;
    $posicaoX_titulo = 465;
    $posterY = 80;
    ...
    if ($posterImage !== false) {
        ...
        $posterHeight = 570;
        $posterX = 30;
        imagecopyresampled($image, $posterImage, $posterX, 115, 0, 0, $posterWidth, $posterHeight, imagesx($posterImage), imagesy($posterImage));
        imagedestroy($posterImage);
    }
    
    $tamanhoMaximoFonteCat = 25;
    $tamanhoMinimoFonteCat = 10;
    $larguraMaximaCategoria = $imageWidth - 625;
    $posicaoX_cat = 620;
    $posicaoY_cat = 330;
    ...
    $maxWidth = $imageWidth - 480;
    $wrappedSinopse = wrapText($sinopse, $fontPath, $fontSize, $maxWidth);
    imagettftext($image, $fontSize, 0, 465, 390, $whiteColor, $fontPath, $wrappedSinopse); 
    
    imagettftext($image, 20, 0, 215, $imageHeight - 30, $whiteColor, $fontPath, "O MELHOR DO STREAMING VOCÊ SÓ ENCONTRA AQUI");
    imagettftext($image, 16, 0, 465, $imageHeight - 180, $whiteColor, $fontPath, "DISPONÍVEL EM DIVERSOS APARELHOS");
    
	    $urlDispositivos = 'https://i.ibb.co/h11C1rff/Design-sem-nome-12.png';
