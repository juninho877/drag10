...
define('LOGO_OVERRIDES', [
    ...
    'América-MG Sub-20' => 'https://image-service.onefootball.com/transform?w=256&dpr=2&image=https://images.onefootball.com/icons/teams/164/9135.png',
    'Cuiaba Sub-20' => 'https://www.ogol.com.br/img/logos/equipas/3220_imgbank_1683624743.png',
    'Bahia U17' => 'https://i.ibb.co/C3VkgMyg/99122.png',
    'Internacional U17' => 'https://www.ogol.com.br/img/logos/equipas/2245_imgbank_1683626195.png',
    'Fortaleza U17' => 'https://upload.wikimedia.org/wikipedia/commons/e/e9/Fortaleza_EC_2018.png',
    'Athletico PR U17' => 'https://upload.wikimedia.org/wikipedia/commons/thumb/4/43/Athletico_Paranaense_%28Logo_2019%29.svg/1200px-Athletico_Paranaense_%28Logo_2019%29.svg.png',
    'Juventude U17' => 'https://www.ogol.com.br/img/logos/equipas/2246_imgbank_1683626208.png',
...
function carregarImagemDeUrl(string $url, int $maxWidth, ?int $maxHeight = null) {
    global $imageCache;
    $maxHeight = $maxHeight ?? $maxWidth; // Se altura não for informada, assume igual à largura

    $cacheKey = md5($url . $maxWidth . 'x' . $maxHeight);
    ...
    // Calcula escala proporcional para caber nos limites fornecidos
...
function carregarEscudo(string $nomeTime, ?string $url, int $maxWidth = 60, ?int $maxHeight = null) {
    $maxHeight = $maxHeight ?? $maxWidth;

    if (!empty($url)) {
        $imagem = carregarImagemDeUrl($url, $maxWidth, $maxHeight);
        if ($imagem) return $imagem;
    }

    return criarPlaceholderComNome($nomeTime, $maxWidth, $maxHeight);
}
...
function obterJogosDeHoje() {
	// Monta os parâmetros
	$parametros = json_decode(file_get_contents('api_parametros.json'), true);
	$api_url = $parametros['api_url'];
	$api_caminho = $parametros['api_caminho'];
	$query_string_base  = $parametros['$query_string_base'];
	$api_token = $parametros['api_token'];

	$json_url = $api_url . '/' . $api_caminho . '?' . $query_string_base . $api_token;
...
function _gerarBannerModel1($userId, $jogos, $grupoJogos, $width, $height, $padding, $heightPorJogo, $fontLiga) {
    ...
    foreach ($grupoJogos as $idx) {
        ...
        $imgEscudo1 = carregarEscudo($time1, $escudo1_url, 140, 140);
    ...
    $xData = centralizarTextoX($width, 45, $fonteData, $dataTexto);
    
    imagettftext($im, 45, 0, $xData, 205, $corBranco, $fonteData, $dataTexto);
...
function _gerarBannerModel2($userId, $jogos, $grupoJogos, $width, $height, $padding, $heightPorJogo, $fontLiga) {
    ...
    $xData = centralizarTextoX($width, 28, $fonteTitulo, $dataTexto);
    
    imagettftext($im, 36, 0, $xTitulo1, 65, $corBranco, $fonteTitulo, $titulo1);
    imagettftext($im, 28, 0, $xData, 105, $corBranco, $fonteTitulo, $dataTexto);
